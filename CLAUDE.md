...
- **Branded coordinate units** (`app/src/utils/units.ts`): `CssPx`, `DevicePx`, `WorldCell`, `Dpr` are `number & Unit<…>`. The coordinate spaces are now **type-enforced** — a `CssPx` cannot be passed where `DevicePx`/`WorldCell` is expected. Raw numbers are branded only at entry boundaries (WASM getters, pointer events, parsed storage) via the constructors (`cssPx`, `devicePx`, `worldCell`, `dpr`); arithmetic drops the brand and is re-branded at the conversion sites (`gridCoords.ts`). This makes the two contracts below compiler-checked, not just comment/test-guarded (interface-audit B).
- **One DPR**: never read `window.devicePixelRatio` raw — use `effectiveDpr(): Dpr` (`app/src/utils/devicePixelRatio.ts`). Canvas sizing, click→cell mapping, and camera device-offset must agree. *Enforced by `dprDiscipline.test.ts`* (guards raw reads the type system can't see; bump its allowlist only for justified raw reads) *and now by the `Dpr` brand.*
